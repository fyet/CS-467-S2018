  ...
  function certificateHandler($type,$date,$email,$fname,$lname,$user){
    ...
    if(is_dir("$directory")){ // If the directory in question exists...
        ...
        $latexContent1 = "  \documentclass{article}
                            ...
                            \pagecolor{yellow!30}
                            \begin{center}
                            \includegraphics{../../images/border_graphic.png}
                            \\vspace{2.5cm}
                            \begin{Huge}
                            \\textbf{\\textit{";
        $latexContent2 = "  }}
        ...
        $latexContent5 = "  }
                            ...
                            \par
                            \bigskip
                            {\\textbf{Authorizing Party Signature:    }}\includegraphics{./signature.png";
        $latexContent6 =    "}\par\\noindent\\rule{18cm}{0.4pt}
                            \par
                            \\end{Large}
                            \\end{flushleft}
        ...
        // Build a full path
        $path = '/var/www/html/award-hub/user-side/' . $directory . '/certificate.pdf';

        // Handle database activities to get the last award
        require('../database-resources/config.php'); // Connect to DB, a new instance is now available via $dbc variable
        ...
        require('../email-handler/emailHandler.php'); // Require our email handler file
